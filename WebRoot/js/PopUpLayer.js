...
var templateSrc = "${ctx}/image/loading.gif"; // 设置loading.gif路径
function tipsWindown(title, content, width, height, drag, time, showbg,
		cssName, backcall) {
	
	$("#windown-box").remove(); // 请除内容
	var width = width >= 1536 ? this.width = 1536 : this.width = width; // 设置最大窗口宽度
	var height = height >= 864 ? this.height = 864 : this.height = height; // 设置最大窗口高度
	if (showWindown == true) {
				...
				.ajaxStart(
						function() {
							$(this)
									.html(
											"<img src='"
													+ templateSrc+"/images/loading.gif' class='loading' />");
});
$.ajax({
